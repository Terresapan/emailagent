...
export default function NewsletterItem({ digest }: NewsletterItemProps) {
  ...
  return (
    <div
      className={`group rounded-xl border transition-all duration-300 ${isExpanded
    ...
    >
      ...
      >
        <div className="flex items-center gap-4">
          <div className={`rounded-lg p-2 transition-colors ${isExpanded ? "bg-primary/20 text-primary" : "bg-white/5 text-muted-foreground group-hover:text-foreground"
            }`}>
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-2 text-xs uppercase tracking-wider text-muted-foreground">
              <span className="font-medium text-muted-foreground">{digest.sender}</span>
            </div>
          </div>
        </div>
        {hasDetails && (
              ...
              <section>
                ...
                <ul className="space-y-2">
                  {summary.industry_news.map((item, i) => (
                    <li key={i} className="text-base text-muted-foreground leading-relaxed pl-4 relative border-l border-white/10 hover:border-primary/50 transition-colors">
                      {item}
                    </li>
                  ))}
                </ul>
