...
import { deleteArchive, ArchivedItem } from "@/lib/api";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Archive, Trash2, ExternalLink, Calendar } from "lucide-react";
import { cn } from "@/lib/utils";
import Link from "next/link";
import { useRouter } from "next/navigation";

interface ArchivesClientProps {
    initialArchives: ArchivedItem[];
}
...
export default function ArchivesClient({ initialArchives }: ArchivesClientProps) {
    ...
    const filteredArchives = activeTab === 'all'
        ? initialArchives
        : initialArchives.filter(item => item.item_type === activeTab);
