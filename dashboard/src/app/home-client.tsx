...
import { triggerProcess, getProcessStatus, archiveItem, Digest, ToolsInsight, HackerNewsInsight, YouTubeInsight } from "@/lib/api";
...
interface HomeClientProps {
  initialDailyDigest: Digest | null;
  initialWeeklyDigest: Digest | null;
  initialToolsInsight: ToolsInsight | null;
  initialHackerNewsInsight: HackerNewsInsight | null;
  initialYoutubeInsight: YouTubeInsight | null;
}
...
export default function HomeClient({
  ...
  initialWeeklyDigest,
  initialToolsInsight,
  initialHackerNewsInsight,
  initialYoutubeInsight,
}: HomeClientProps) {
  const router = useRouter();
  const [activeView, setActiveView] = useState<ViewType>("daily");
  
  // Use props as initial state, but allow updates via router.refresh()
  const dailyDigest = initialDailyDigest;
  const weeklyDigest = initialWeeklyDigest;
  const toolsInsight = initialToolsInsight;
  const hackerNewsInsight = initialHackerNewsInsight;
  const youtubeInsight = initialYoutubeInsight;
  ...
  // Sticky Header Logic - optimized with passive listener
  useEffect(() => {
    const handleScroll = () => {
    ...
    // Check if passive events are supported (browsers usually do now)
      ...
      const pollInterval = setInterval(async () => {
        ...
        try {
          ...
          if (status.status === "completed") {
            ...
            handleRefresh(); // Refresh server data
            setTimeout(() => {
        ...
        <MotionItem className="relative z-20 flex items-center justify-between border-b border-white/10 pb-6 mt-4">
          <div className="flex gap-8">
            <button
              ...
              className={`text-sm font-medium tracking-[0.2em] uppercase transition-all duration-300 ${activeView === "daily" ? "text-accent" : "text-muted-foreground hover:text-white"}`}
            >
              Intelligence
            </button>
            <button
              onClick={() => setActiveView("producthunt")}
              className={`text-sm font-medium tracking-[0.2em] uppercase transition-all duration-300 ${activeView === "producthunt" ? "text-accent" : "text-muted-foreground hover:text-white"}`}
            >
              Product Hunt
            </button>
            <button
              onClick={() => setActiveView("hackernews")}
              className={`text-sm font-medium tracking-[0.2em] uppercase transition-all duration-300 ${activeView === "hackernews" ? "text-accent" : "text-muted-foreground hover:text-white"}`}
            >
              HackerNews
            </button>
            <button
              onClick={() => setActiveView("youtube")}
              className={`text-sm font-medium tracking-[0.2em] uppercase transition-all duration-300 ${activeView === "youtube" ? "text-accent" : "text-muted-foreground hover:text-white"}`}
            >
              YouTube
            </button>
            <button
              onClick={() => setActiveView("weekly")}
              className={`text-sm font-medium tracking-[0.2em] uppercase transition-all duration-300 ${activeView === "weekly" ? "text-accent" : "text-muted-foreground hover:text-white"}`}
            >
              Strategy
            </button>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3 text-muted-foreground text-xs tracking-widest uppercase">
              ...
              <Button variant="ghost" size="icon" onClick={handleRefresh} className="hover:text-white hover:bg-white/5" title="Refresh">
